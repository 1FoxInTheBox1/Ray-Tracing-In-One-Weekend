...
mesh::mesh(const char *filename, point3 position, vec3 scale, shared_ptr<material> mat) 
{
    load_file(filename, position, scale, triangles, mat);
}

mesh::mesh(const char *filename, point3 position, shared_ptr<material> mat) 
{
    load_file(filename, position, vec3(1, 1, 1), triangles, mat);
}
